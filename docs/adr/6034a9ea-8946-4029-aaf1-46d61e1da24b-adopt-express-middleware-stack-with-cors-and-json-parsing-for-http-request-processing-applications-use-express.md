...
1. MUST: Applications MUST use express.json() middleware via app.use(express.json()) to parse JSON request bodies before route handlers execute

## Policy Block

- MUST Applications MUST use express.json() middleware via app.use(express.json()) to parse JSON request bodies before route handlers execute
