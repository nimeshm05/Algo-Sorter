# Define Express HTTP Service Boundaries with CORS and Route Handlers: Postgresql Connections Enforce
...
- **R-CORS-001** MUST: PostgreSQL connections MUST enforce SSL with rejectUnauthorized: false for cloud-hosted databases.
...
- **R-CORS-005** MUST: All database connection strings MUST be sourced from process.env variables with SSL configuration.
- **R-CORS-006** MUST: Validate environment variables at application startup before initializing database connections or starting HTTP server.
- **R-CORS-007** SHOULD: Structure route handlers with async/await and send JSON responses using res.send({...}) for consistent API contracts.
...
grep -r 'postgresql://\|mysql://\|mongodb://' server/src/ | grep -v process.env | grep -v '.env.example'
```
...
- All database operations use parameterized queries with values array
